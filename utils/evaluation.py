...
#Adapted from https://github.com/openai/guided-diffusion/blob/main/evaluations/evaluator.py
...
def generate_50k_samples(
    ...
    print(f"Generating 50K samples in {sample_folder}...")
    with torch.inference_mode():
        for i in tqdm(range(n_classes)):
            imgs = hmar.generate(50, i, cfg=cfg, top_p=top_p, top_k=top_k, more_smooth=more_smooth,
                                 num_samples=num_samples, g_seed=i, mask=mask, mask_schedule=mask_schedule)
            for j in range(50):
                torchvision.utils.save_image(imgs[j], os.path.join(sample_folder,f"{i}_{j}.png"))

def compute_metrics(
        sample_npz: str ="samples", ref_npz : str = "VIRTUAL_imagenet256_labeled.npz", 
        download_url: str = "https://openaipublic.blob.core.windows.net/diffusion/jul-2021/ref_batches/imagenet/256/",
):
    #want to download the reference npz if not available
    if not os.path.exists(ref_npz):
        os.system(f'wget {download_url}{ref_npz}')

    print("Computing metrics...")
    FID, sFID, IS, prec, recall = _compute_metrics(ref_npz, sample_npz)
    print(f"FID: {FID}, sFID: {sFID}, IS: {IS}, prec: {prec}, recall: {recall}")
