...
# Adapted from https://github.com/FoundationVision/VAR/blob/main/utils/misc.py
...
class SyncPrint(object):
    def __init__(self, local_output_dir, sync_stdout=True):
        ...
        fname = os.path.join(local_output_dir, f'stdout-{dist.get_rank()}.txt' if sync_stdout else f'stderr-{dist.get_rank()}.txt')
...
class WandbLogger(object):
    def __init__(self, args: arg_util.Args):
        super().__init__()
        self.wandb_initialized = False
        self.args = args

    def initialized(self):
        return self.wandb_initialized
    ...
    def set_step(self, step=None):
        if dist.is_master() and self.args.log_to_wandb:
    ...
    def update(self, head=None, step=None, **kwargs):
        if dist.is_master() and self.args.log_to_wandb:
            if not self.wandb_initialized:
                self.init_wandb(self.args)
            log_items = {}
    ...
    def log_tensor_as_distri(self, tag, tensor1d, step=None):
        if dist.is_master() and self.args.log_to_wandb:
            ...
            if step is None:
                step = self.step
            try:
                wandb.log({tag: wandb.Histogram(tensor1d)}, step=step)
            except Exception as e:
                print(f'[log_tensor_as_distri wandb.log failed]: {e}')
    
    def log_images(self, tag, imgs, nrow=8, step=None):
        if dist.is_master() and self.args.log_to_wandb:
            if not self.wandb_initialized:
                self.init_wandb(self.args)
            if step is None:
            ...
            try:
                ...
                wandb.log({tag: [wandb.Image(imgs)]}, step=step)
            except Exception as e:
                print(f'[log_images wandb.log failed]: {e}')
                
    def log_file(self, file, policy='now'):
        if dist.is_master() and self.args.log_to_wandb:
            if not self.wandb_initialized:
                self.init_wandb(self.args)
            wandb.save(file, policy=policy)
...
def delete_old_ckpts(out_dir_path, pattern='ckpt-epoch-*.pth', keep=5):
    file = os.path.join(out_dir_path, pattern)
    all_ckpt = glob_with_latest_modified_first(file)
    try:
        for ckpt in all_ckpt[keep:]:
            os.remove(ckpt)
    except Exception as e:
        print('[delete_old_ckpts] failed:', e)

def auto_resume(args: arg_util.Args, pattern='ckpt*.pth') -> Tuple[List[str], int, int, dict, dict]:
    info = []
    file = os.path.join(args.experiment_dir_path, pattern)
    all_ckpt = glob_with_latest_modified_first(file)
    if len(all_ckpt) == 0:
...
def auto_resume_finetune(args: arg_util.Args, pattern='ckpt*.pth') -> Tuple[List[str], int, int, dict, dict]:
    info = []
    
    #get the checkpoint for the base model to finetune
    file = os.path.join(args.base_experiment_dir_path, pattern)
    all_ckpt = glob_with_latest_modified_first(file)
    if len(all_ckpt) == 0:
        raise FileNotFoundError(f'[auto_resume] No base ckpt found @ {file}')
    else:
    ...
    #get the finetune checkpoint if it exists
    file = os.path.join(args.experiment_dir_path, pattern)
    all_ckpt = glob_with_latest_modified_first(file)
    if len(all_ckpt) == 0:
        info.append(f'[auto_resume] No finetune ckpt found @ {file}, starting from scratch')
        ep, it = 0, 0
        trainer, args = {}, {}
    else:
        info.append(f'[auto_resume] load finetune ckpt from @ {all_ckpt[0]} ...')
        ckpt = torch.load(all_ckpt[0], map_location='cpu')
        ep, it = ckpt['epoch'], ckpt['iter']
        trainer, args = ckpt['trainer'], ckpt['args']
        
    return info, ep, it, base_ckpt, trainer, args
