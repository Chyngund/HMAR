...
# Adapted from https://github.com/FoundationVision/VAR/blob/main/models/basic_var.py
...
class SelfAttention(nn.Module):
    def __init__(
        ...
        patch_nums_sq = [p**2 for p in patch_nums]
        self.block_sparse_attn = BlockSparseAttention(patch_nums_sq)
        ...
        # This is only used for benchmarking purposes to compare performance to VAR
        self.caching, self.cached_k, self.cached_v = False, None, None
    
    # This is only used for benchmarking purposes to compare performance to VAR
    def kv_caching(self, enable: bool): self.caching, self.cached_k, self.cached_v = enable, None, None

    def forward(self, x, using_block_sparse_attn=False, attn_bias=None):
        B, L, C = x.shape
        ...
        # use flash attention when we do not need a block diagonal mask
        using_flash = (
            self.using_flash
            and not using_block_sparse_attn
        ...
        # This is only used for benchmarking purposes, to compare the inference performance to VAR
        if self.caching:
        ...
        elif using_block_sparse_attn:
            assert (
                self.attn_drop == 0.0
            )  # custom kernel does not currently support dropout
            oup = (
                self.block_sparse_attn(q, k, v, self.scale)
                .transpose(1, 2)
                .reshape(B, L, C)
            )
        else:
...
class AdaLNSelfAttn(nn.Module):
    ...
    def forward(
        self, x, cond_BD, using_block_sparse_attn=False, attn_bias=None
